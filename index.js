...
//Iteração sobre todas as perguntas
for(const item of perguntas){
    //Clona o modelo para cada pergunta
    ...
    for(let resposta of item.respostas){
        //Clona o elemento de definição de termo para cada resposta
        const dt = quizItem.querySelector('dl dt').cloneNode(true)
        //define o texto da resposta
        dt.querySelector('span').textContent = resposta

        //Define para clicar somente em 1 resposta em cada pergunta
        dt.querySelector('input').setAttribute('name', 'pergunta-' + perguntas.indexOf(item))
        //pegando o valor dentro do input e interando em respostas 0,1,2,3,4
        dt.querySelector('input').value = item.respostas.indexOf(resposta)


        //Iterando as respostas acertos e erros
    ...
    //Remove o primeiro item de definição (placeHolder)
    quizItem.querySelector('dl dt').remove()

    //Adiciona a pergunta e suas respostas ao quiz
